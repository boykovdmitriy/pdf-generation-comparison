...
import { pdfMakeClientGeneration } from './pdfMakeClientGeneration';
import {
  Button,
  LoadingPlaceholder,
  PdfViewer,
...
const PdfMakeDocPreview = ({ data }: Props) => {
  const { onDownload } = useDownloadPdf({ data, engine: 'pdfmake' });
  ...
  useEffect(() => {
    pdfMakeClientGeneration(data).then((res) => {
      setDocument(res);
    });
  }, [data]);
  ...
  return (
    <div className="space-y-4">
      <div className="space-x-3 flex">
        <Button disabled onClick={onDownload} variant="secondary" size="full">
...
export default PdfMakeDocPreview;
